...
struct ClaudeUsageView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            if let error = registry.claudeError {
                Text(error.message).font(.system(size: 12)).foregroundStyle(.secondary)
    ...
    private func window(_ limit: RateLimit, title: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title).font(.system(size: 11, weight: .semibold, design: .monospaced)).foregroundStyle(.secondary)
            Text("\(Int(limit.remainingPercent.rounded()))%")
                .font(.system(size: 36, weight: .semibold, design: .rounded))
            Text("remaining").font(.system(size: 12)).foregroundStyle(.secondary)
